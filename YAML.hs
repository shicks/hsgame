...
--     f sp _ Null = addSpace sp . showString "null"
--     f sp i (Leaf x) = addSpace sp . showLeaf i x
--     f sp i (List ns) =
--         nl sp [ indent i . showString "-" . f True (i+1) n | n <- ns ]
--     f sp i (Map ps) =
--         nl sp $ map (showPair i) ps
--     indent i = showString $ replicate i ' '
--     showLeaf i x | '\n' `elem` x = showString' x -- use line-break version?
--                  | otherwise     = showString' x
--     showPair i (Leaf k,v) | not $ '\n' `elem` x = indent i . showString' k .
--                                                   showString ":" . f True (i+1) y
--     showPair i (k,v) = indent i . showChar '?' . f True (i+1) k . showChar '\n'
--                        indent i . showChar ':' . f True (i+1) k
--     nl _ [] = id
--     nl sp xs = (if sp then ('\n':) else id) .
--                foldr1 (\x y -> x . showChar '\n' . y) xs
--     addSpace True  = showChar ' '
--     addSpace False = id
